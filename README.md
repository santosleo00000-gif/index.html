...
## Painel multi-marca (novo)

A partir de agora, acessar a URL do Railway abre um **painel de login**, não mais a página de
status simples. Cada marca que você cadastrar (aba "Cadastrar marca" na tela de login) tem:

- Login próprio, isolado das outras marcas.
- Tela de Leads, Pedidos e Conversas.
- **Configurações** — cada marca cola suas próprias credenciais de Shopify/WhatsApp/Mercado
  Pago/E-mail/IA direto no painel (nada de mexer no Railway pra cada marca nova). As chaves
  ficam criptografadas no banco.
- **Disparos** — envio manual de mensagem (WhatsApp ou e-mail) pra todos os leads da marca,
  com `{{nome}}` sendo trocado pelo nome de cada um automaticamente.

**Duas novas variáveis obrigatórias no Railway** (já vêm com valores prontos no `.env.example`,
é só copiar): `ENCRYPTION_KEY` e `JWT_SECRET`.

### O que ainda não mudou (Fase 2 futura)
As automações de webhook (PIX automático quando o pedido é criado, atualização de rastreio,
chatbot respondendo sozinho) ainda usam as variáveis de ambiente globais do Railway — ou seja,
por enquanto elas seguem valendo só pra a marca original (BLY!). As credenciais que você cadastra
no painel, por enquanto, alimentam a tela de Configurações e os **Disparos manuais**. Ligar os
webhooks automáticos a cada marca cadastrada no painel (multi-tenant completo) é o próximo passo.

O construtor visual de fluxos de chatbot (tipo Tintim) também fica para uma próxima etapa.
