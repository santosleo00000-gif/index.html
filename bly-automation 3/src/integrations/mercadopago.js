...
async function createPixCharge({ orderId, amount, description, payerEmail, expirationMinutes = 30 }) {
  if (!process.env.MP_ACCESS_TOKEN) {
    console.warn('[MercadoPago] Ignorando createPixCharge - MP_ACCESS_TOKEN ainda não configurado.');
    return null;
  }
...
function verifyWebhookSignature({ xSignature, xRequestId, dataId }) {
  if (!process.env.MP_WEBHOOK_SECRET) {
    console.warn('[MercadoPago] MP_WEBHOOK_SECRET ainda não configurado - rejeitando webhook por segurança.');
    return false;
  }
