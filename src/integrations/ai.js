...
async function transcribeAudio(buffer, mimeType = 'audio/ogg') {
  if (!process.env.TRANSCRIPTION_API_KEY) {
    console.warn('[IA] Transcrição ignorada - TRANSCRIPTION_API_KEY ainda não configurado.');
    return '[áudio recebido, mas transcrição ainda não configurada]';
  }
...
async function classifyAndReply({ message, context }) {
  if (!process.env.ANTHROPIC_API_KEY) {
    console.warn('[IA] Resposta ignorada - ANTHROPIC_API_KEY ainda não configurado.');
    return { intent: 'outro', reply: 'Recebemos sua mensagem! Em breve alguém da equipe responde por aqui.' };
  }
