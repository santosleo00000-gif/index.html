...
function warnNotConfigured(action) {
  console.warn(`[WhatsApp] Ignorando "${action}" - WHATSAPP_ACCESS_TOKEN / WHATSAPP_PHONE_NUMBER_ID ainda não configurados.`);
}
...
async function sendText(to, body) {
  if (!isConfigured()) return warnNotConfigured('sendText');
...
async function sendTemplate(to, templateName, languageCode = 'pt_BR', components = []) {
  if (!isConfigured()) return warnNotConfigured('sendTemplate');
  return client.post('/messages', {
    messaging_product: 'whatsapp',
    to,
    type: 'template',
...
async function downloadMedia(mediaId) {
  if (!isConfigured()) {
    warnNotConfigured('downloadMedia');
